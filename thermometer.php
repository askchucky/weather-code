...
$commaDecimal = false; // false=decimal is period, true=decimal is comma//------------ end settings --------------
...
if (isset($_REQUEST['wx']) and strtolower($_REQUEST['wx']) == 'wlcom')  { $wxSoftware = 'WLCOM';  } // testing
if (isset($_REQUEST['wx']) and strtolower($_REQUEST['wx']) == 'awn')  { $wxSoftware = 'AWN';  } // testing
...
if ($wxSoftware == 'WL' or $wxSoftware == 'WLCOM') { // Process WeatherLink
      if(!isset($WLrealtime)) { // get from WLtags.php
	     if (file_exists($SITE['WXtags'])) {
				 global $WX; include_once($SITE['WXtags']); 
			 }
	   $inUOM = 'F';
		 $curtemp = 70;
		 $mintemp = 60;
...
if ($wxSoftware == 'AWN') { // Process WeatherLink
   if (file_exists($SITE['WXtags'])) {
				 global $WX; include_once($SITE['WXtags']); 
	 }
	   $inUOM = 'F';
		...
		if($commaDecimal) {
			$WX['outsideTemp'] = str_replace(',','.',$WX['outsideTemp']); // revert to real decimal
			$WX['lowOutsideTemp'] = str_replace(',','.',$WX['lowOutsideTemp']); // revert to real decimal
			$WX['hiOutsideTemp'] = str_replace(',','.',$WX['hiOutsideTemp']); // revert to real decimal
		}
		if ($inUOM == 'F' and $UOM == 'C') {
		  $curtemp = FtoC($WX['outsideTemp'],1);
		  $mintemp = FtoC($WX['lowOutsideTemp'],1);
